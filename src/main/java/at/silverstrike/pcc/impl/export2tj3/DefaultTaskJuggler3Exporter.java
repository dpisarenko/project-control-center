...
class DefaultTaskJuggler3Exporter implements TaskJuggler3Exporter {
    ...
    private static final Logger LOGGER =
        LoggerFactory.getLogger(DefaultTaskJuggler3Exporter.class);
    ...
    public void setInjector(final Injector anInjector) {
        if (anInjector != null) {
            ...
            embeddedFileReader =
                    anInjector.getInstance(EmbeddedFileReader.class);
        }
    }
    ...
    private CharSequence getEffortInfo(final ControlProcess process) {
        return effortTemplate.replace(EFFORT, formatDouble(process
                .getAverageCaseEffort()));
    }
    ...
    private CharSequence getStartDateTime(final ControlProcess process,
        ...
        if (childProcesses != null) {
            for (final ControlProcess childProcess : childProcesses) {
                childProcessDefinitions +=
                        getTaskInformation(childProcess, process);
            }
        }

        final String taskDefinition =
                taskTemplate.replace(ID, formatLong(process.getId()))
                        .replace(NAME, shortenName(process.getName())).replace(
                                START_DATE_TIME,
                                getStartDateTime(process, parent)).replace(
                                PRIORITY, formatInt(process.getPriority()))
                        .replace(RESOURCE_ALLOCATIONS,
                                getEffortAllocations(process)).replace(
                                EFFORT_INFO, getEffortInfo(process)).replace(
                                CHILD_TASKS, childProcessDefinitions);
    ...
    private String subsituteResourcePlaceHolders(
        ...
        final String resourceDefinition =
                resourceDefinitionTemplate.replace(ABBREVIATION,
                        resource.getAbbreviation()).
                        replace(RESOURCE_ID, resource.getId().toString()).
                        replace(
                                DAILY_LIMIT_IN_HOURS,
        ...
        final String projectHeader =
                projectTemplate.replace(PLACEHOLDER_PROJECT_NAME,
                        ...
                        PLACEHOLDER_COPYRIGHT,
                                this.projectExportInfo.getCopyright()).replace(
