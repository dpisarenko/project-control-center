...
class DefaultEditingProcessPanel extends Panel implements EditingProcessPanel {
	...
	private static final String VMSG_MAX_EFFORT = TM
			.get("editingprocesspanel.30-save-validation-error-" + "MAX_EFFORT");
	...
	protected void saveButtonClicked() {
		final SaveValidationResult validationResult = validateSaveRelevantInputs();

		if (!SaveValidationResult.EVERYTHING_OK.equals(validationResult)) {
			String errorMessage = saveErrorMessagesByValidationResults
					.get(validationResult);

			if (isEmpty(errorMessage)) {
				errorMessage = TM
						.get("editingprocesspanel.28-unexpected-error");
			}
	...
	private Integer getPriority() {
		...
		try {
			priority = Integer.parseInt((String) this.priorityTextBox
					.getValue());
	...
	private void createDependencyControls(final GridLayout aGrid) {
		...
		final Button addButton = new Button(
				get("editingprocesspanel.25-add-dependency-button"));
	...
	private void createEffortEstimateControls(final GridLayout aGrid) {
		final Label minEffortLabel = new Label(
				TM.get("editingprocesspanel.17-min-effort-label"));
		...
		final Label maxEffortLabel = new Label(
				TM.get("editingprocesspanel.18-max-effort-label"));
	...
	private void createHandOffControls(final GridLayout grid) {
		...
		controlSubjectComboBox.setItemCaptionMode(ITEM_CAPTION_MODE_PROPERTY);
	...
	private void fillWorkerComboBox() {
		...
		for (final Worker worker : workers) {
			...
			final String template = TM
					.get("editingprocesspanel.14-resource-combobox-item");
			...
			final String abbreviatedName = StringUtils.abbreviate(resourceText,
					30);
	...
	private void createPriorityControls(final GridLayout aGrid) {
		final Label priorityLabel = new Label(
				get("editingprocesspanel.22-priority-caption"));
	...
	private Worker getWorker(final ControlProcess aProcess) {
		final List<ResourceAllocation> allocations = aProcess
	...
	private void initSaveErrorMessagesByValidationResults() {
		saveErrorMessagesByValidationResults = new HashMap<SaveValidationResult, String>();
	...
	private void setButtonEnabledness() {
		if (process != null) {
			...
			final boolean notDeleted = !ProcessState.DELETED.equals(process
					.getState());
	...
	private void setupSaveRelevantListeners() {
		...
		final SaveButtonRelevantListener saveRelevantListener = new SaveButtonRelevantListener(
				saveButton);
		final List<AbstractField> saveRelevantComponents = new LinkedList<AbstractField>();
	...
	private SaveValidationResult validateEffort() {
		...
		final double minEffortInMinutes = getEffortInMinutes(minEffort,
				minTimeUnitComboBox);
		final double maxEffortInMinutes = getEffortInMinutes(maxEffort,
				maxTimeUnitComboBox);
